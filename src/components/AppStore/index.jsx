...
const AppStore = () => {
  ...
  const [activeTabText, setActiveTabText] = useState(tabsList[0].tabId)
  ...
  const getActiveTabApps = searchedApps => {
    return searchedApps.filter(
      eachSearchedApp => eachSearchedApp.category === activeTabText,
    )
  }

  const getSearchResults = () => {
    return apps.filter(eachApp => {
      return eachApp.appName.toLowerCase().includes(searchInput.toLowerCase())
    })
  }
      ...
      <div className="app-store">
        ...
        <ul className="tabs-list">
          {tabsList.map(eachTab => (
            <TabItem
              tabDetails={eachTab}
              isActive={activeTabText === eachTab.tabId}
              setActiveTabId={setActiveTabText}
              key={eachTab.tabId}
            />
          ))}
        </ul>
